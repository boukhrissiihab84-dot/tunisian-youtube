// =====================================================================
// TUNISIAN TUBE - APP.JS (VIDEOS LOADING FIX)
// =====================================================================
...
function applyGridSize() {
    ...
    document.querySelectorAll(".grid").forEach(g => {
        g.style.gridTemplateColumns = `repeat(auto-fill, minmax(${s.gridSize || 280}px, 1fr))`;
    });
}
...
function setLang(v) {
    ...
    if (input) input.placeholder = trans[v] || trans.en;
}
...
function loadToggles() {
    ...
    if (fontSel) fontSel.value = s.fontSize || "normal";
    const gridSel = document.getElementById("gridSelect");
    if (gridSel) gridSel.value = s.gridSize || "280";
    const qSel = document.getElementById("qualitySelect");
    if (qSel) qSel.value = s.quality || "auto";
}
...
function closeAuth() {
    document.getElementById("authOv")?.classList.remove("active");
...
function toggleAuthMode() {
    ...
    if (sa) sa.style.display = isSignUp ? "flex" : "none";
...
function previewAvatar(e) {
    ...
    if (f) {
        ...
        r.onload = x => {
            ...
            document.getElementById("signupAvatar").innerHTML = `<img src="${tempAvatar}"><input type="file" accept="image/*" onchange="previewAvatar(event)">`;
        };
        r.readAsDataURL(f);
    }
}
...
function loadAccountPage() {
    ...
    const wrap = document.getElementById("accAvatarWrap");
    if (wrap) {
        wrap.innerHTML = user.avatar 
            ? `<img src="${user.avatar}"><input type="file" accept="image/*" onchange="changeAccPic(event)">` 
            : `<i class="fas fa-camera"></i><input type="file" accept="image/*" onchange="changeAccPic(event)">`;
    }
...
function saveAccount() {
    ...
    const o = document.getElementById("accOldPass").value;
...
function loadChannelPage() {
    ...
    document.getElementById("channelBigAvatar").innerHTML = user.avatar 
        ? `<img src="${user.avatar}">` 
...
function saveChannel() {
    const ch = {
        ...
        category: document.getElementById("channelCat").value,
        link: document.getElementById("channelLink").value.trim(),
        social: document.getElementById("channelSocial").value.trim()
    };
...
function toggleSub(ch) {
    ...
    S.s(k, s);
    refreshActionsLarge(currentVid);
}
function toggleSubFromPage() {
    const ch = document.getElementById("mChanLarge").textContent;
    toggleSub(ch);
}
...
function postCmt(id) {
    if (!user) { openAuth(); return; }
    const inp = document.getElementById("cmtInputLarge");
    if (!inp) return;
...
function renderCmtsLarge(id) {
    const s = document.getElementById("cmtSectionLarge");
    ...
    let list = "";
    if (!cmts.length) {
        list = `<p style="color:var(--text3);font-size:13px;padding:8px 0">No comments yet. Be the first! 💬</p>`;
    } else {
        cmts.forEach(c => {
            const delBtn = user && user.id === c.uid ? `<button class="cmt-del" onclick="delCmt('${id}','${c.id}')"><i class="fas fa-trash"></i></button>` : "";
            list += `<div class="cmt-item"><div class="cmt-avatar" style="width:32px;height:32px;font-size:12px">${c.name ? c.name[0].toUpperCase() : "U"}</div><div style="flex:1"><span class="cmt-user">@${c.name}</span>${delBtn}<p class="cmt-text">${c.text}</p><span class="cmt-time">${c.time}</span></div></div>`;
        });
    }
...
function shareV(id) {
    ...
    if (navigator.share) {
        navigator.share({ title: "Cours Tounsi", url: u });
    } else {
...
function downloadV(id) {
    ...
    const sf = document.getElementById("dlSaveFrom");
    ...
    if (sf) sf.href = `https://en.savefrom.net/#url=${encoded}`;
...
function initApp(raw) {
    if (!Array.isArray(raw) || raw.length === 0) {
        ...
        document.getElementById("grid").innerHTML = "<p style='color:red;text-align:center;grid-column:1/-1;padding:40px'>⚠️ Ma fammach vidéos! Thabbet fi tounes_courses.json fi GitHub.</p>";
        return;
    }
    allVideos = raw.map(v => {
...
// FETCH JSON WITH TIMESTAMP
fetch("tounes_courses.json?v=" + Date.now(), { cache: "no-store" })
    .then(r => {
    ...
    .catch(err => {
        ...
        document.getElementById("grid").innerHTML = "<p style='color:red;text-align:center;grid-column:1/-1;padding:40px'>⚠️ Fichier tounes_courses.json ma l9inehch! Sobbou fi GitHub.</p>";
    });
...
function buildChips() {
    ...
    cats.forEach(c => {
        fb.innerHTML += `<button class="f-chip" onclick="filterChipHome('${c}')">${c}</button>`;
    });
}
    ...
    g.insertAdjacentHTML("beforeend", batch.map(v => `
        <div class="card" onclick="navigate('video', {id:'${v.id}'})">
            <div class="card-thumb">
                <img src="${v.thumb}" loading="lazy" onerror="this.src='https://img.youtube.com/vi/${v.id}/0.jpg'">
                <span class="card-badge">${v.topic}</span>
            </div>
            <div class="card-body">
                <div class="card-title">${v.title}</div>
                <div class="card-ch"><i class="fas fa-user-circle"></i> ${v.channel}</div>
...
function showAll() {
    ...
    if (si) si.value = "";
    document.querySelectorAll(".side-btn,.f-chip").forEach(b => b.classList.remove("active"));
    document.querySelector(".f-chip")?.classList.add("active");
    document.querySelectorAll("#catList .side-btn")[0]?.classList.add("active");
    navigate("home");
}
...
function apply() {
    ...
    if (currentFilter.search) {
        r = r.filter(v => (v.title + v.channel + v.topic + v.category).toLowerCase().includes(currentFilter.search));
    }
    setListAndRender(r);
}
...
function loadVideoPage(v) {
    ...
    if (!v && arguments[0]?.id) v = allVideos.find(x => x.id === arguments[0].id);
    if (!v) return;
    currentVid = v.id;
    ...
    refreshActionsLarge(v.id);
    renderCmtsLarge(v.id);
    renderRelatedVideos(v.category, v.id);
    if (S.g("settings")?.history) {
        let h = S.g("history") || [];
        if (!h.includes(v.id)) { h.unshift(v.id); S.s("history", h); }
    }
}

function refreshActionsLarge(id) {
    const l = isLiked(id);
    const lc = getLikes(id).length;
    const ch = document.getElementById("mChanLarge").textContent;
    const s = isSub(ch);
    ...
    if (b) {
        b.className = "subscribe-btn" + (s ? " subscribed" : "");
        b.innerHTML = s ? '<i class="fas fa-check"></i> Subscribed' : '<i class="fas fa-bell"></i> Subscribe';
    }
}
...
function renderRelatedVideos(cat, cid) {
    ...
    if (!final.length) {
        list.innerHTML = `<p style="color:var(--text3);font-size:13px;padding:12px">Ma fammach videos similaires</p>`;
        return;
    }
    list.innerHTML = final.map(v => `
        <div class="related-item" onclick="navigate('video',{id:'${v.id}'})">
            ...
            <div class="related-info">
                ...
                <div class="related-ch"><i class="fas fa-user-circle"></i> ${v.channel}</div>
                <div class="related-cat">${v.topic}</div>
            </div>
        </div>
    `).join("");
}
...
function renderGridPage(gid, list, msg) {
    ...
    if (!g) return;
    if (!list.length) {
        g.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-inbox"></i><h2>${msg}</h2></div>`;
        return;
    }
...
document.addEventListener("keydown", e => {
    if (e.key === "Escape") { closeAuth(); closeDlModal(); }
});
